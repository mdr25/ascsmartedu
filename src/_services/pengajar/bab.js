...
export async function getBabByMapelId(mapelId) {
  ...
  try {
    const response = await api.get("/teacher/bab", {
...
export async function createBab(data) {
  const token = localStorage.getItem("token");
  try {
    const response = await api.post("/teacher/bab", data, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error) {
...
export async function getBabDetail(id) {
  try {
    const response = await api.get(`/teacher/bab/${id}`);
    return response.data;
  } catch (error) {
    console.error(
...
export async function updateBab(id, data) {
  try {
    const response = await api.put(`/teacher/bab/${id}`, data);
    return response.data;
  } catch (error) {
    console.error(
      "Gagal mengupdate bab:",
      error.response?.data || error.message
    );
    throw error;
  }
...
export async function deleteBab(id) {
  try {
    const response = await api.delete(`/teacher/bab/${id}`);
    return response.data;
  } catch (error) {
    console.error(
      "Gagal menghapus bab:",
