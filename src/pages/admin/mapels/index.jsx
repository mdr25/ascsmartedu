...
const MapelIndex = () => {
  ...
  const [editMapelData, setEditMapelData] = useState({ id: null, nama_mapel: "" });
  const [showEditModal, setShowEditModal] = useState(false);
  const [isEditing, setIsEditing] = useState(false); // ✅ Tambahan
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      setIsEditing(false); // ✅ Clear loading
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <nav className="flex items-center text-sm text-gray-500 mb-4 space-x-2">
        <button onClick={() => navigate("/admin/classes")} className="text-teal-700 hover:underline">
          {classData?.jenjang_kelas?.nama_jenjang || "Kelas"}
        </button>
        <span>&gt;</span>
        <button onClick={() => navigate(`/admin/classes/${classId}`)} className="text-teal-700 hover:underline">
        ...
        <div className="border border-teal-500 rounded-xl p-4 bg-gray-100">
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {mapelList.map((mapel) => (
                <div key={mapel.id} className="relative bg-white p-3 rounded shadow-sm">
                  <div className="flex justify-between items-center">
                    <button
                      onClick={() =>
                        navigate(`/admin/classes/${classId}/mapel/${mapel.id}/bab`)
                      }
                    ...
                    <div className="relative">
                      <button onClick={() => toggleDropdown(mapel.id)} className="text-gray-500 hover:text-gray-700">
                        ⋮
                      </button>
                      {activeDropdown === mapel.id && (
                        <div className="absolute right-0 mt-2 w-28 bg-white border rounded shadow z-20">
                          <button
                            onClick={() => {
                              setEditMapelData({ id: mapel.id, nama_mapel: mapel.nama_mapel });
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white rounded-lg p-6 w-96 shadow-lg">
            ...
            <input
              ...
              onChange={(e) =>
                setEditMapelData((prev) => ({ ...prev, nama_mapel: e.target.value }))
              }
              className="border p-2 rounded w-full text-sm mb-4"
