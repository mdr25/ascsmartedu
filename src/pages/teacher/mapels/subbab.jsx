  ...
  const fetchSubbabs = async () => {
    try {
      ...
      if (classId) {
        ...
        console.warn(
          "classId tidak ditemukan dari relasi mata_pelajaran.classes"
        );
      }
    } catch (err) {
              ...
              {subbabs.map((item) => (
                ...
                >
                  <span
                    onClick={() =>
                      navigate(
                        `/teacher/classes/${classId}/mapel/${lessonId}/bab/${babId}/subbab/${item.id}/content`
                      )
                    }
