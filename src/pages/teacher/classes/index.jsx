...
export default function StudentClasses() {
    ...
    const fetchClasses = async () => {
      try {
        ...
        const uniqueClasses = Array.from(
          new Map(
            data.map((item) => [
              `${item.id}-${item.jenjang_kelas?.id || "x"}`,
              item,
            ])
          ).values()
        );
  ...
  const filteredClasses =
    activeTab === "Semua"
      ? classes
      : classes.filter((cls) => cls?.jenjang_kelas?.nama_jenjang === activeTab);

  return (
    <div className="container mx-auto p-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold">Kelas yang Anda ajar</h2>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400">
          Tidak ada kelas yang anda ajar.
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-700">
                    <span className="text-gray-500">Siswa:</span> {studentCount}
                  </p>
                  <p className="text-sm text-gray-700">
